import { logger, thought, branch, dialogue, server, message, envelope } from '.'
import * as bot from '..'
...
export namespace state {
  ...
  export class State implements IOptions {
    ...
    message: message.Message = message.blank()
    ...
    get path () {
      if (!this.dialogue) this.dialogue = dialogue.create()
    ...
    respondEnvelope (options?: envelope.IOptions) {
      ...
      if (!pending) {
        ...
        pending = envelope.create(options, this)
