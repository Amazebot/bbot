...
/** @todo update tests when thought controller uses bot constructor */
...
describe('[thought]', () => {
  ...
  beforeEach(async () => {
    ...
    bot.adapter.slots.nlu = mock.adapters.nlu
    ...
    describe('.processes', () => {
      ...
      describe('understand.validate', () => {
        ...
        it('false if non-text message', async () => {
          const enter = messages.enter(createUser())
      ...
      describe('.respond.validate', () => {
        ...
        it('false without message adapter', async () => {
          delete bot.adapter.slots.message
          const thoughts = new Thoughts(new State({ message }))
          thoughts.b.envelope.compose('test')
          const valid = await thoughts.processes.respond.validate()
          expect(valid).to.equal(false)
      ...
      describe('.respond.action', () => {
        context('on success', () => {
          it('dispatches envelope to message adapter', async () => {
            ...
            sinon.assert.calledWithExactly(
              bot.adapter.slots.message.dispatch,
              envelope
            )
          })
        })
      ...
      describe('.remember.validate', () => {
        it('remembers user when branch matched', async () => {
          ...
          expect(bot.memory.users[message.user.id]).to.eql(message.user)
        })
        it('false if no adapter', async () => {
          delete bot.adapter.slots.storage
          const thoughts = new Thoughts(new State({ message }))
          thoughts.b.matchingBranches = [new CustomBranch(() => true, () => null)]
